...
out:
	...
	if _, exists := server.connections[id]; exists {
		...
		// There is already a connection with the same ID. Close the new one immediately with 1008.
		server.error(fmt.Errorf("client %v already exists, closing duplicate client", id))
		_ = conn.WriteControl(websocket.CloseMessage,
			websocket.FormatCloseMessage(websocket.ClosePolicyViolation, "client already exists"),
			time.Now().Add(server.timeoutConfig.WriteWait))
		_ = conn.Close()
		return
	}
